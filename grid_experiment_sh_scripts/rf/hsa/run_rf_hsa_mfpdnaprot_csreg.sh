...
#SBATCH -J 20250629_hsa_mfpdnaprot_rf_csreg_experiment      #job name
...
#SBATCH --output=scratch/anticancer-synergy-prediction-scratch/experiments/20250629_hsa_rf/mfpdnaprot_rf_csreg_%a_slurm.out           #output file
#SBATCH --error=scratch/anticancer-synergy-prediction-scratch/experiments/20250629_hsa_rf/mfpdnaprot_rf_csreg_%a_slurm.err            #error file

source env/bin/activate

SCRATCH="scratch/anticancer-synergy-prediction-scratch/"
export PYTHONPATH="${PYTHONPATH}:~/cancer-drug-synergy-prediction/"
PYOF=$SCRATCH"experiments/20250629_hsa_rf/mfpdnaprot_rf_csreg"$SLURM_ARRAY_TASK_ID".txt"
PLOT=$SCRATCH"experiments/20250629_hsa_rf/mfpdnaprot_rf_csreg"
...
python3 models/run/run_rf_models.py --score 'HSA' --use_mfp --use_dna --use_prot --use_csreg --output_fp $PLOT"metrics.csv" > $PYOF
