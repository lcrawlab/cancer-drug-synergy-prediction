...
# Fit the knn model for binary classification
# INPUT:
#   X_train: np.ndarray
#   y_train: np.ndarray
# OUTPUT:
#   model: KNNModelBC
def fit_knn_bc_model(X_train, y_train):
...
# Evaluate the knn model for binary classification
# INPUT:
#   model: KNNModelBC
#   X_test: np.ndarray
#   y_test: np.ndarray
# OUTPUT:
#   fold_metrics: list
def evaluate_knn_bc_model(model, X_test, y_test):
    # Evaluation
    with torch.no_grad():
...
# Fit the knn model for regression on comboscore or percent growth
# INPUT:
#   X_train: np.ndarray
#   y_train: np.ndarray
# OUTPUT:
#   model: KNNModelRegression
def fit_knn_reg_model(X_train, y_train):
    # Initialize and train the model
    model = KNNModelRegression()
    model.fit(X_train, y_train)
    print("Model trained")
...
# Evaluate the knn model for regression on comboscore or percent growth
# INPUT:
#   model: KNNModelRegression
#   X_test: np.ndarray
#   y_test: np.ndarray
# OUTPUT:
#   fold_metrics: list
def evaluate_knn_reg_model(model, X_test, y_test):
    # Evaluation
    with torch.no_grad():
...
# Run the knn model, one fold, so can be parallelized
# INPUT:
...
def process_fold(i, train_index, test_index, X, y, args):
    ...
    model = None
    if args.use_bc:
        model = fit_knn_bc_model(X_train, y_train)
        fold_metrics = evaluate_knn_bc_model(model, X_test, y_test)
    elif args.use_csreg:
        model = fit_knn_reg_model(X_train, y_train)
        fold_metrics = evaluate_knn_reg_model(model, X_test, y_test)
    elif args.use_pgreg:
        model = fit_knn_reg_model(X_train, y_train)
        fold_metrics = evaluate_knn_reg_model(model, X_test, y_test)
    else:
